...
contract ListBusiness {
  ...
  function addDN(
    ...
    listDNs.push(
      ListDN(_businessOwner, _name, _country, _facebook, _website, _linkedin, _focusArea, _password)
    );
  }
    ...
    )
  {
    ...
    for (uint256 i = 0; i < listDNs.length; i++) {
  ...
  function getProfile(address _businessOwner)
    public
    view
    returns (
      address businessOwner,
  ...
  function checkDN(address _businessOwner, string memory _password)
    public
    view
    returns (uint256 x)
  {
    for (uint256 i = 0; i < listDNs.length; i++) {
      if (
        (listDNs[i].businessOwner == _businessOwner) &&
        (keccak256(bytes(listDNs[i].password)) == keccak256(bytes(_password)))
      ) {
        return 1;
      }
    }
  ...
  function checkExistDN(address _businessOwner) public view returns (uint256 x) {
    for (uint256 i = 0; i < listDNs.length; i++) {
      if (listDNs[i].businessOwner == _businessOwner) {
        return 1;
      }
