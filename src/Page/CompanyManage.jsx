import react ,{useContext}from "react"; 
import HeaderCompany from './../Components/HeaderCompany';
import { myContract } from './../Api/Const';
import { RiCloseFill } from 'react-icons/Ri';
import { Context } from '../Context/Context';

const CompanyManage  = () => {
    const {addrCompany, setAddrCompany,addr, setAddr} = useContext(Context)
    console.log(addr)
    var web3 = new Web3(Web3.providers.HttpProvider('http://localhost:7545'));
    web3.eth.getAccounts().then(console.log);
    // var myContract = new web3.eth.Contract(
    //    contract,
    //     '0x305FA8E23654E95fA1B6883E802272AA61F1a3a5'
    // );
    function addProfCompany(e) {
        e.preventDefault()
        setAddrCompany($("#_addrCompany").val())
        myContract.methods.addBusinessProfile($("#_addrCompany").val(), $("#_nameCompany").val(), $("#_nationalCompany").val(), $("#_focusArea").val(), $("#_linkedinCompany").val(), $("#_fbCompany").val(), $("#_webCompany").val())
                .send({
                  from: $("#_addrCompany").val(),
                  gas: 3000000
                });
        // navigate("/mycv");
       
        
        
      };
      function showListCompany(){
        myContract.methods
        .getListBusiness($("#_addrCompany").val())
        .call()
        .then((result) => console.log(result));
      }
    return ( 
        <>
            <HeaderCompany />
            <div className="min-h-screen min-w-full bg-primary pb-[8rem]">
                <div className="h-[5vh] bg-primary "></div>
                <div className="w-[70%] h-[100%] mx-auto pt-[4rem] bg-white rounded-md flex flex-col pb-[4rem]">
                    <div className="flex px-10 justify-between">
                        <div className="flex">
                            {/* <img src="data:image/png;base64," alt="" className=" w-[10rem]  block object-cover" /> */}
                            <h1 className="ml-6 mt-4 text-[2.5rem] font-bold">KMS Technology</h1>
                        </div>
                        <button className="w-[10%] h-12 p-2 mt-5 rounded-lg text-white bg-orange-btn" 
                            onClick={addProfCompany}>Save</button>
                    </div>
                    <hr className="w-[60%] mx-auto mt-8 h-1 bg-primary rounded-md"/>
                    <div className="mt-12 px-[6rem]">
                        <div className="flex">
                            <div className="w-[10%]">
                                <label className="text-2xl ">Name: </label>
                            </div>
                            <input type="text" name="nameCompany" id="_nameCompany" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="KMS Technology" />
                        </div>                
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">National:</label>
                            </div>
                            <input type="text" name="nationalCompany" id="_nationalCompany" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" 
                            placeholder="Vietnam" />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Focus Area:</label>
                            </div>
                            <input type="text" name="focusArea" id="_focusArea" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">LinkedIn:</label>
                            </div>
                            <input type="text" name="linkedinCompany" id="_linkedinCompany" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.linkedin.com/company/kms ..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Facebook:</label>
                            </div>
                            <input type="text" name="fbCompany" id="_fbCompany" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Address:</label>
                            </div>
                            <input type="text" name="addrCompany" id="_addrCompany" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Website:</label>
                            </div>
                            <input type="text" name="webCompany" id="_webCompany"  className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://kms-technology.com" />
                        </div>
                    </div>
                    <button type="submit" id="btn_show" onClick={showListCompany} className="h-[2.75rem] w-[8rem] mt-8 text-white font-medium bg-secondary rounded-[30px]">Show</button>
                </div>
            </div>
        </>
     );
}
 
export default CompanyManage;