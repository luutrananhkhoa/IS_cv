import react ,{useContext}from "react"; 
import HeaderCompany from './../Components/HeaderCompany';
import { myContract } from './../Api/Const';
import { RiCloseFill } from 'react-icons/Ri';
import { Context } from '../Context/Context';
import { useNavigate, Link } from 'react-router-dom';

const CompanyManage  = () => {
    const {addrCompany, setAddrCompany,addr, setAddr,profileBusiness, setProfileBusiness} = useContext(Context)
    console.log(addr)

    var web3 = new Web3(Web3.providers.HttpProvider('http://112.78.4.41:8545'));
    web3.eth.getAccounts().then(console.log);
    const navigate=useNavigate();

    function addProfCompany(e) {
        e.preventDefault()
        setAddrCompany($("#_addrCompany").val())
        myContract.methods.addBusinessProfile($("#_addrCompany").val(), $("#_nameCompany").val(), $("#_nationalCompany").val(), $("#_focusArea").val(), $("#_linkedinCompany").val(), $("#_fbCompany").val(), $("#_webCompany").val())
                .send({
                  from: $("#_addrCompany").val(),
                  gas: 3000000
                });
        navigate("/companyprofile");
      };

      function showListCompany(){
        myContract.methods
        .getBusinessProfile($("#_addrCompany").val())
        .call()
        .then((result) => console.log(result))
        .then((result) => setProfileBusiness(result));
      }
    return ( 
        <>
            <HeaderCompany />
            <div className="min-h-screen min-w-full bg-primary pb-[8rem]">
                <div className="h-[5vh] bg-primary "></div>
                <div className="w-[70%] h-[100%] mx-auto pt-[4rem] bg-white rounded-md flex flex-col pb-[4rem]">
                    <div className="flex px-10 justify-between">
                        <div className="flex">
                            {/* <img src="data:image/png;base64," alt="" className=" w-[10rem]  block object-cover" /> */}
                            <h1 className="ml-6 mt-4 text-[2.5rem] font-bold">My Company</h1>
                        </div>
                        <button className="w-[10%] h-12 p-2 mt-5 rounded-lg text-white bg-orange-btn hover:bg-primary active:text-orange-btn" 
                            onClick={addProfCompany}>Save</button>
                    </div>
                    <hr className="w-[60%] mx-auto mt-8 h-1 bg-primary rounded-md"/>
                    <div className=" flex flex-col mt-12 px-[6rem]">
                        <div className="flex">
                            <div className="w-[50%] items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl ">Name </label>
                                </div>
                                <input type="text" name="nameCompany" id="_nameCompany" className="border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="KMS Technology" />
                            </div>                
                            <div className=" w-[50%] items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl">National</label>
                                </div>
                                <input type="text" name="nationalCompany" id="_nationalCompany" className="border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none" 
                                placeholder="Vietnam" />
                            </div>
                        </div>
                        <div className="flex">
                            <div className=" w-[50%] mt-10 items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl">Business</label>
                                </div>
                                <input type="text" name="focusArea" id="_focusArea" className="border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                            </div>
                            <div className="w-[50%] mt-10 items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl">LinkedIn</label>
                                </div>
                                <input type="text" name="linkedinCompany" id="_linkedinCompany" className="border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.linkedin.com/company/kms ..." />
                            </div>
                        </div>
                        <div className="flex">
                            <div className="mt-10 w-[50%] items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl">Facebook</label>
                                </div>
                                <input type="text" name="fbCompany" id="_fbCompany" className=" border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                            </div>
                            
                            <div className="mt-10 w-[50%] items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl">Website</label>
                                </div>
                                <input type="text" name="webCompany" id="_webCompany"  className="border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none " placeholder="https://kms-technology.com" />
                            </div>
                        </div>
                        <div className="flex">
                        <div className="mt-10 w-[50%] items-center">
                                <div className="w-[10%]">
                                    <label className="text-xl">Address</label>
                                </div>
                                <input type="text" name="addrCompany" id="_addrCompany" className=" border-2 w-[60%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                            </div>
                        </div>
                        
                    </div>
                    {/* <button type="submit" id="btn_show" onClick={showListCompany} className="h-[2.75rem] w-[8rem] mt-8 text-white font-medium bg-secondary rounded-[30px]">Show</button> */}
                </div>
            </div>
        </>
     );
}
 
export default CompanyManage;