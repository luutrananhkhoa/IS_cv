import react from "react"; 
import HeaderCompany from './../Components/HeaderCompany';
import { RiCloseFill } from 'react-icons/Ri';

const CompanyManage  = () => {
    return ( 
        <>
            <HeaderCompany />
            <div className="min-h-screen min-w-full bg-primary pb-[8rem]">
                <div className="h-[5vh] bg-primary "></div>
                <div className="w-[70%] h-[100%] mx-auto pt-[4rem] bg-white rounded-md flex flex-col pb-[4rem]">
                    <div className="flex px-10 justify-between">
                        <div className="flex">
                            {/* <img src="data:image/png;base64," alt="" className=" w-[10rem]  block object-cover" /> */}
                            <h1 className="ml-6 mt-4 text-[2.5rem] font-bold">KMS Technology</h1>
                        </div>
                        <button className="w-[10%] h-12 p-2 mt-5 rounded-lg text-white bg-orange-btn">Save</button>
                    </div>
                    <hr className="w-[60%] mx-auto mt-8 h-1 bg-primary rounded-md"/>
                    <div className="mt-12 px-[6rem]">
                        <div className="flex">
                            <div className="w-[10%]">
                                <label className="text-2xl ">Name: </label>
                            </div>
                            <input type="text" name="" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="KMS Technology" />
                        </div>
                        
                        {/* <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl ">Title:</label>
                            </div>
                            <input type="text" name="" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="Agile, Innovative & Excellent Global ..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl ">Introduce:</label>
                            </div>
                            <textarea name="" className="ml-[10rem] border-2 w-[40%] h-[10rem] px-5 py-2 rounded-lg border-slate-300  outline-none resize-none" placeholder="Established in 2009, KMS Technology is a U.S.-based engineering and services company with development centers in Vietnam. KMS Technology is trusted by international clients for the superior...." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Logo:</label>
                            </div>
                            <input type="file" accept="image/*" className="ml-[10rem] w-[40%] px-5 py-2 rounded-lg  outline-none"/>
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Photo:</label>
                            </div>
                            <input type="file" accept="image/*" className="ml-[10rem] w-[40%] px-5 py-2 rounded-lg  outline-none"/>
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Skilk:</label>
                            </div>
                            <div className="flex flex-col">
                                <div className="">
                                    <input type="text" name=""  className="ml-[10rem] border-2 w-[30%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="Java" />
                                    <button className="w-[20%] h-10 ml-10 p-2 rounded-lg text-white bg-orange-btn">Add</button>
                                </div>
                                <div className="flex ml-[10rem] mt-4">
                                    <div className="flex">
                                        <div className="w-fit flex items-center h-[2rem] m-2 text-white text-center bg-secondary rounded-md px-[1rem]">
                                            <p className="leading-[2rem]">Javascript</p>
                                            <RiCloseFill size="1.5rem" className="ml-2 text-primary hover:cursor-pointer"/>
                                        </div>
                                        <div className="w-fit flex items-center h-[2rem] m-2 text-white text-center bg-secondary rounded-md px-[1rem]">
                                            <p className="leading-[2rem]">.NET</p>
                                            <RiCloseFill size="1.5rem" className="ml-2 text-primary hover:cursor-pointer"/>
                                        </div>
                                    </div>
                                </div>
                            </div>}
                        </div> */}
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">National:</label>
                            </div>
                            <input type="text" name="" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" 
                            placeholder="Vietnam" />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">LinkedIn:</label>
                            </div>
                            <input type="text" name="" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.linkedin.com/company/kms ..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Facebook:</label>
                            </div>
                            <input type="text" name="" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://www.facebook.com/kmstechnoly..." />
                        </div>
                        <div className="flex mt-10">
                            <div className="w-[10%]">
                                <label className="text-2xl">Website:</label>
                            </div>
                            <input type="text" name="" className="ml-[10rem] border-2 w-[40%] px-5 py-2 rounded-lg border-slate-300 outline-none" placeholder="https://kms-technology.com" />
                        </div>
                    </div>
                </div>
            </div>
        </>
     );
}
 
export default CompanyManage;